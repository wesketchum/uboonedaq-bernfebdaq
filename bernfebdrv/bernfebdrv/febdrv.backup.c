...
memset(FEB_present,0,256);
 ...
 printf("Scanning connected FEBs..\n");
 for(num=0;num<255;num++)
 {
  memcpy(macs[nclients],dstmac,6);
  macs[nclients][5]=num;
  
  if(sendcommand(macs[nclients],FEB_SET_RECV,FEB_VCXO[nclients],hostmac)<=0) return 0;  
  if(recvfromfeb(10000))  { 
      for(int i=0;i<5;i++) printf("%02x:",macs[nclients][i]);
      printf("%02x ",macs[nclients][5]);
      printf("%s\n",rpkt.Data);  nclients++;} // client reply received
      FEB_present[num]=1;
 }
 printf("\n%d FEBs found.\n",nclients);
 return nclients; 
...
for(int f=0; f<nclients;f++) //loop on all connected febs : macs[f][6]
 {
  ...
  if(curl) {
    /* First set the URL that is about to receive our POST. This URL can
       ...
       data. */ 
    curl_easy_setopt(curl, CURLOPT_URL, "http://localhost:8086/db/data/series?u=root&p=root");
...
//   memcpy(ptr,&fs,sizeof(fs)); ptr+=sizeof(fs);
    ...
    sprintf(str,"name=feb%d&columns=evrate&points=%f",fs.mac[5],fs.evtrate);
    printf("%s\n",str);
 //   curl_easy_setopt(curl, CURLOPT_POSTFIELDS, str);
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, "name=feb&columns=evrate&points=777");
    printf("After setopt\n");
...
//	 printf("Corrected T0=%u ns, T1=%u ns T0_ref=%u ns  T1_ref=%u ns \n",ts0,ts1,ts0_ref,ts1_ref);
        evt=getnextevent();
        if(evt==0) {printdate();  printf("Buffer overrun for FEB S/N %d !! Aborting.\n",macs[f][5]); continue;} 
        evt->ts0=ts0;
        evt->ts1=ts1;
	evt->flags=0;
